...
def _score_customers(veh_repr, cust_repr, edge_emb, owner_bias, lookahead):
    ...
    # unbiased=False avoids NaN on single-element slices; clamp+nan_to_num for safety
    def z_norm(s):
...
def forward(dyna):
    ...
    # 3. Main loop
    # Note: _encode_customers is NOT called before the loop.
    # The environment starts with new_customers=True so the first iteration handles encoding.
    actions, logps, rewards = [], [], []
    
    while not dyna.done:
        # Encode (or re-encode) customers whenever a new batch appears (DVRPTW)
        if dyna.new_customers:
            self._encode_customers(dyna.nodes, dyna.cust_mask)
        # self.cust_repr: N × L_c × D, cached until next new_customers event
...
│                                                                      │
│ Note: _encode_customers is NOT called here. The environment starts   │
│ with new_customers=True; the first loop iteration handles encoding.  │
└─────────────────────────────────────────────────────────────────────┘
                                 │
                                 ▼
┌─────────────────────────────────────────────────────────────────────┐
...
│  │ PHASE 5: Selection                                            │  │
│  ├──────────────────────────────────────────────────────────────┤  │
│  │ if all customers masked for a sample:                        │  │
│  │   unmask index 0 (prevents NaN in log_softmax)               │  │
│  │ compat ← compat.clone(); compat[cur_veh_mask] ← -inf         │  │
│  │ logp ← log_softmax(compat, dim=-1)                           │  │
│  │   Shape: N × L_c                                              │  │
│  │                                                                │  │
│  │ if greedy:                                                    │  │
...
VECTRA(
    ...
    cust_k=None,            # k-NN for graph encoder (None = full attention)
