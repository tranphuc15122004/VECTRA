...
| Method | Avg Tour Length | Computation Time | Feasibility Rate |
|--------|----------------|------------------|------------------|
| OR-Tools | 450.2 | 120s | 100% |
| LKH | 448.5 | 180s | 100% |
| Attention (base) | 465.3 | 0.5s | 95% |
| + Fleet Encoder | 458.7 | 0.6s | 97% |
| + Edge Features | 455.2 | 0.7s | 98.5% |
| **VECTRA (ours)** | **451.3** | **0.8s** | **99.2%** |
...
```python
from MODEL.model.marl_model import EdgeEnhencedLearner
from problems import DynEnvTW

# 1. Create model
model = EdgeEnhencedLearner(
    cust_feat_size=5,
    veh_state_size=4,
    model_size=128,
    layer_count=3,
...
# 2. Load data
data = torch.load('data/dvrptw_n100m5_1000.pyth')
...
```bibtex
@article{vectra2024,
  title={VECTRA: Vehicle-Edge-Customer Temporal Routing Anticipation for Dynamic Vehicle Routing},
  author={Your Name},
