...
class EdgeEnhencedLearner(nn.Module):
    def __init__(self, cust_feat_size, veh_state_size, model_size = 128,
        ...
        self.cust_encoder    = GraphEncoder(layer_count, head_count, model_size, ff_size)

        self.fleet_encoder   = FleetEncoder(layer_count, head_count, model_size, ff_size, k = fleet_k)
    ...
    def _encode_fleet(self, vehicles):
        fleet_edges = self._build_fleet_edges(vehicles)
        fleet_enc = self.fleet_encoder(vehicles, fleet_edges)
    ...
    def _repr_vehicle(self, vehicles, veh_idx, mask):
        ...
        """
        fleet_repr = self._encode_fleet(vehicles) #.size() = N x L_v x D
    ...
    def step(self, dyna):
        ...
        compat = self._score_customers(veh_repr, self.cust_repr, edge_emb, owner_bias, lookahead)
