...
def _verify_routes_cost(data, env_cls, env_params, routes, model_costs, rollouts = 1):
    ...
    print(
        "Route verification: replay_mean={:.4f}, model_mean={:.4f}, max_abs_diff={:.6f}".format(
...
def main(args):
    ...
    print(
        "Cost summary: normalized_cost_mean={:.4f}, raw_replay_cost_mean={:.4f}".format(
            costs.mean().item(),
    ...
    for idx in range(min(3, costs.numel())):
        print(
            "  Instance #{} | normalized_cost={:.4f} | raw_replay_cost={:.4f}".format(
                idx,
                costs[idx].item(),
    ...
    print(
        "Skipped customers: total={} | per_instance={}".format(
            total_skipped,
            [d["missing_count"] for d in route_diagnostics[: min(10, len(route_diagnostics))]],
        )
    )
    print("Constraint violations: total_tw={} | total_appearance={}".format(total_tw_viol, total_appear_viol))
    for idx in range(min(3, len(constraint_diagnostics))):
        rep = constraint_diagnostics[idx]
        print(
            "  Instance #{} | tw_violations={} | appearance_violations={}".format(
                idx,
