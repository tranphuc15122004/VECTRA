...
Recent neural-routing research has advanced complementary axes. PolyNet learns diverse solution strategies
\cite{hottung2025polynet}, while MVMoE and RouteFinder pursue transfer across VRP variants through task-level experts and generalized problem representations
\cite{zhou2024mvmoe,berto2025routefinder}. RL4CO further makes policies and evaluation protocols easier to compare through a common benchmark framework
\cite{berto2025rl4co}. The routing methods mainly concern fully observed route construction or task-level generalization; online DVRPTW still requires per-event fleet coordination under unrevealed demand and candidate-specific anticipation.
...
Neural combinatorial optimization has progressed from recurrent constructive policies \cite{nazari2018reinforcement} to attention-based routing models \cite{kool2019attention}. Recent work also improves solution search: PolyNet learns complementary decoding strategies to generate diverse complete solutions \cite{hottung2025polynet}. These methods strengthen representation learning and solution diversity, but they do not by themselves distinguish why one customer is preferable at a particular online decision event.

A second line seeks policies that transfer across static VRP variants. MVMoE uses hierarchically gated experts to improve multi-task and zero-shot performance \cite{zhou2024mvmoe}, and RouteFinder represents variants through global attributes, mixed-batch training, and adapters \cite{berto2025routefinder}. RL4CO complements this work with modular environments, policies, learning algorithms, and benchmark coverage for reproducible neural combinatorial optimization \cite{berto2025rl4co}. MVMoE and RouteFinder adapt across tasks or complete instances, rather than asynchronous fleet decisions with unrevealed requests.

For event-driven DVRP, reinforcement learning avoids repeatedly solving an optimization model online \cite{akkerman2025comparison,konovalenko2024optimizing}. The sMMDP formulation of MARDAM schedules one vehicle at a time \cite{bono2021mardam}; related multi-agent policies use sequential decisions, shared attention, or learned coordination \cite{zhang2020multi,shahbazian2025knowledge}. These methods motivate scalable fleet representations, while edge-aware encoders capture pairwise routing consequences \cite{meng2025eformer}. COAST is complementary to solution diversity and task-level generalization: it separates, within each online decision, immediate vehicle--customer compatibility, fleet-relative ownership, and candidate-conditioned downstream impact. This decomposition provides action-specific evidence that a state-value critic, which is shared by all candidates in the same state, cannot provide.
...
\end{aligned}
\]
Here $b_{k_tj}$ is service-start time, including any wait before $e_j$. We use soft time windows: late service is penalized, rather than masked, as in multi-vehicle soft-window routing \cite{zhang2020multi,bono2021mardam}.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.96\textwidth]{figures/COAST_final.drawio.png}
...
\end{aligned}
\]
Thus ownership is a learned coordination bias across vehicles, not explicit vehicle-to-vehicle message passing. In parallel, the LookaheadHead produces a learned candidate-conditioned signal,
\[
S_{t,j}^{\mathrm{look}}=\operatorname{MLP}_{\mathrm{look}}
([\mathbf h_t^{\mathrm{act}},\mathbf c_{t,j},\mathbf e_{t,k_t,j}^{\mathrm{emb}}]),
\]
which is optimized end-to-end and is not a calibrated value or $Q$-function.
...
\section{Conclusions and Future Work} \label{sec:futurework}
COAST decomposes each event-driven customer decision into local edge-aware compatibility, fleet-relative ownership, and candidate-conditioned lookahead, then adaptively fuses these signals into a unified policy. Matched-instance benchmarks and targeted ablations show improved routing quality and complementary, context-sensitive contributions, particularly as fleet-coordination demands increase. Future work will strengthen lookahead across operating regimes through confidence-aware fusion or lightweight auxiliary guidance, together with multi-seed validation and route-level cost analysis.
