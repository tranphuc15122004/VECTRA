...
def main(args):
        ...
        from externals._lkh import lkh_solve
        import time
        print("Running LKH3 in parallel across instances...")
        print(f"Dataset has {raw_data.batch_size} instance(s)")
        print(f"Nodes shape: {raw_data.nodes.shape}")
        print(f"Vehicles: {raw_data.veh_count}, Capacity: {raw_data.veh_capa}, Speed: {raw_data.veh_speed}")
        
        # raw_data is already in original (unnormalized) scale since it was
        # cloned from `data` BEFORE data.normalize().  No denormalization needed.
        # The LKH3 solver receives the original-scale coordinates, demands,
        # time windows, service times, and vehicle capacity/speed directly.
        print(f"raw_data veh_capa={raw_data.veh_capa}, veh_speed={raw_data.veh_speed}")
        print(f"raw_data x range: [{raw_data.nodes[0,:,0].min():.2f}, {raw_data.nodes[0,:,0].max():.2f}]")
        print(f"raw_data y range: [{raw_data.nodes[0,:,1].min():.2f}, {raw_data.nodes[0,:,1].max():.2f}]")
        print(f"raw_data demand range: [{raw_data.nodes[0,:,2].min():.2f}, {raw_data.nodes[0,:,2].max():.2f}]")
        print(f"raw_data time range: [{raw_data.nodes[0,1:,3].min():.2f}, {raw_data.nodes[0,1:,4].max():.2f}]")
