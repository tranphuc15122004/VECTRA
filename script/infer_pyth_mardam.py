#!/usr/bin/env python3
"""Infer all .pyth datasets with mardam model, with top-down trial batching."""
...
def estimate_batch_size(data, config_path, model_weight, max_batch_size, min_batch_size, is_sample):
    ...
    Uses mardam's AttentionLearner (from MODEL/infer_mardam.py).
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Infer all .pyth datasets with mardam model (top-down trial batching)."
    )
    ...
    parser.add_argument("--batch-size", type=int, default=None,
                        help="Override trial (disables top-down estimation)")
    ...
    for r in all_results:
        if r["status"] == "ok":
            print(f"  {r['dataset']}: mean={r['mean']:.4f} std={r['std']:.4f} time={r.get('inference_time', 'N/A')}s [{r['count']} instances]")
        else:
