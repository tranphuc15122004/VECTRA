...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--infer-script", type=Path, default=Path("script/infer_lkh.py"))
    parser.add_argument(
        "--python-executable",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of worker processes used inside each LKH inference run",
    )
...
def _build_subprocess_env(instance_workers: int) -> dict[str, str]:
    env = os.environ.copy()
    env.setdefault("MKL_SERVICE_FORCE_INTEL", "1")
    env.setdefault("OMP_NUM_THREADS", "1")
    env.setdefault("MKL_NUM_THREADS", "1")
    env["MARDAM_LKH_WORKERS"] = str(max(1, instance_workers))
    return env
...
def run_one(
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env=_build_subprocess_env(args.instance_workers),
    )
...
def main() -> int:
    ...
    if args.parallel > 1:
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            futures = {
                executor.submit(run_one, args, csv_path, datasets_root, per_case_root, logs_root): (idx, csv_path)
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if row.get("status") != "ok":
                        failed += 1
                        print(f"[{idx}/{len(csv_files)}]  -> FAILED: {csv_path} - {row.get('error_message', '')}")
                        if args.fail_fast:
                            break
                    else:
                        print(f"[{idx}/{len(csv_files)}]  -> OK: {csv_path}")
                except Exception as e:
                    failed += 1
                    print(f"[{idx}/{len(csv_files)}]  -> EXCEPTION: {csv_path} - {e}")
                    if args.fail_fast:
                        break
    else:
        for idx, csv_path in enumerate(csv_files, start=1):
